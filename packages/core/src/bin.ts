...
export function getBridge(): BinaryBridge {
    if (!bridge) {
        throw new TvmClientError(1, "TON Client binary bridge isn't set.");
    }
    return bridge;
}
...
export class BinaryBridge {
    ...
    private bindingRequired(): Promise<LibraryBinding> {
        ...
        if (this.loading === undefined) {
            return Promise.reject(
                new TvmClientError(1, "TON Client binary library isn't set."),
            );
        }
        return new Promise((resolve, reject) => {
            this.loading?.push({
    ...
    private syncLibraryRequired(): SyncBinaryLibraryWithParams {
        ...
        throw new TvmClientError(1, "TON Client binary library does not support sync calls.");
    }
